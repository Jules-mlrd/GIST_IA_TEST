...
import { listTxtFiles } from '@/lib/listTxtFiles';
import fs from 'fs';
...
export async function POST(req: Request) {
  try {
    ...
    const pdfKeys = await listPdfFilesInS3(process.env.AWS_BUCKET_NAME || '');
    ...
    if (!foundFile) {
      for (const txt of txtFiles) {
        const txtName = txt.replace('.txt', '');
        if (normalize(resolvedMessage).includes(normalize(txtName))) {
          foundFile = { name: txtName, type: 'txt', key: txt };
          break;
        }
      }
    }
    ...
    if (foundFile) {
      ...
      if (foundFile.type === 'pdf') {
        ...
        const filePath = path.join(txtFolder, foundFile.key);
